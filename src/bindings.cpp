...
py::array_t<float> py_matmul_tiled(py::array_t<float> A, py::array_t<float> B) {
    py::buffer_info buf_A = A.request();
    py::buffer_info buf_B = B.request();
...
py::array_t<float> py_matmul_vectorized(py::array_t<float> A, py::array_t<float> B) {
    py::buffer_info buf_A = A.request();
    py::buffer_info buf_B = B.request();
...
PYBIND11_MODULE(cuda_matmul_backend, m) {
    ...
    m.def("matmul_tiled", &py_matmul_tiled, "Thread Tiled (Register Cached) CUDA Matrix Multiplication");
    m.def("matmul_vectorized", &py_matmul_vectorized, "Float4 Vectorized Memory Access CUDA Matrix Multiplication");
}
